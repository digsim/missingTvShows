...
class MainImpl(Main):
    ...
    def __init__(self):
        ...
        self.__tvdbdatabse = join(self._Main__USER_CONFIG_DIR, self.config.get("Config", "tvdbdb"))
    ...
    def getArguments(self, argv):
        ...
        parser = argparse.ArgumentParser(prog='missingtvshows',
        ...
        sync_parser.add_argument("-f", "--force-update", help="Force the update of the local TVDB Database", required=False,
                            action="store_true", dest='forceupdate')
        sync_parser.add_argument("-o", "--offline",
                            help="Force Offline mode, even if the script thinks that some entries needs to be refreshed",
                            required=False, action="store_true", dest='forcelocal')
        sync_parser.add_argument("-c", "--csv", help="Produce CSV output files", required=False, action="store_true",
                            dest='producecsv')
    ...
    def __doSyncCommand(self):
        ...
        :return: void
        """
        tvshows = TVShows(self.__tvdbdatabse, self.__api_key, self.__dbdialect, self.__database, self.__dbuser, self.__dbpasswd, self.__dbhostname, self.__dbport)
        print('Acquiring necessary TV-Shows information')
        unwatched_finished_shows, unwatched_unfinished_shows, watchedsome_unfinished_shows, watchedsome_finished_shows = tvshows.getSeriesInformation()
        utilities.print_konsole(unwatched_finished_shows, unwatched_unfinished_shows, watchedsome_unfinished_shows,watchedsome_finished_shows)
        if self.__produceCVS:
            utilities.save_CSV(unwatched_finished_shows, unwatched_unfinished_shows, watchedsome_unfinished_shows,watchedsome_finished_shows)
