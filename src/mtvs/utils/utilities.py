...
def save_CSV(unwatched_finished_shows, unwatched_unfinished_shows, watchedsome_unfinished_shows,
...
def _write_CSV(series, filename):
    ...
    with f:
        ...
        for show in series:
            writer.writerow([show['SeasonId'], show['Title'].encode("utf-8"), show['Season'], show['NbDownloaded'],
                             show['NbAvailable'], show['MissingEpisodes']])
        f.close()

def print_konsole(unwatched_finished_shows,  unwatched_unfinished_shows,  watchedsome_unfinished_shows,  watchedsome_finished_shows):
    sys.stdout.write('\n')
    print(Fore.RED + '##############################################################')
